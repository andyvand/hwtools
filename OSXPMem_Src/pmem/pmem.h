...
// Write as many bytes as requested in uio->resid (starting from uio->offset)
// and copy them from the userspace buffer in the uio.
static kern_return_t pmem_write(dev_t dev, struct uio *uio, __unused int rw);
