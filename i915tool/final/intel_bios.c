			    ...
			    struct bdb_header *bdb)
{
	...
	panel_fixed_mode = calloc(1, sizeof(*panel_fixed_mode));
	if (!panel_fixed_mode)
		return;
	...
	temp_mode = calloc(1, sizeof(*temp_mode));
			  ...
			  struct bdb_header *bdb)
{
	...
	if (general) {
		...
		if (block_size >= sizeof(*general)) {
			...
			fprintf(stderr, "crt_ddc_bus_pin: %d\n", bus_pin);
			if (bus_pin >= 1 && bus_pin <= 6)
				dev_priv->crt_ddc_pin = bus_pin;
		} else {
			fprintf(stderr, "BDB_GD too small (%d). Invalid.\n",
				  block_size);
		}
	}
}
			  ...
			  struct bdb_header *bdb)
{
	...
	if (!p_defs) {
		fprintf(stderr, "No general definition block is found, unable to construct sdvo mapping.\n");
		return;
	}
	...
	for (i = 0; i < child_device_num; i++) {
		...
		if (p_child->dvo_port != DEVICE_PORT_DVOB &&
			...
			fprintf(stderr, "Incorrect SDVO port. Skip it \n");
			continue;
		}
		fprintf(stderr, "the SDVO device with slave addr %2x is found on"
				" %s port\n",
				p_child->slave_addr,
		...
		if (!p_mapping->initialized) {
			...
			fprintf(stderr, "SDVO device: dvo=%x, addr=%x, wiring=%d, ddc_pin=%d, i2c_pin=%d, i2c_speed=%d\n",
				      p_mapping->dvo_port,
			...
			fprintf(stderr, "Maybe one SDVO port is shared by "
					 "two SDVO device.\n");
		}
		if (p_child->slave2_addr) {
			/* Maybe this is a SDVO device with multiple inputs */
			/* And the mapping info is not added */
			fprintf(stderr, "there exists the slave2_addr. Maybe this"
	...
	if (!count) {
		...
		fprintf(stderr, "No SDVO device info is found in VBT\n");
	}
	return;
}
		       ...
		       struct bdb_header *bdb)
{
	...
	dev_priv->child_dev = calloc(1, sizeof(*p_child) * count);
