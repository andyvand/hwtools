				      ...
				      enum pipe pipe)
{
	...
	reg = FDI_TX_CTL(pipe);
	val = I915_READ(reg);
	if (!(val & FDI_TX_PLL_ENABLE)) fprintf(stderr, "FDI TX PLL assertion failure, should be active but is disabled\n");
}
				      ...
				      enum pipe pipe)
{
	...
	reg = FDI_RX_CTL(pipe);
	val = I915_READ(reg);
	if (!(val & FDI_RX_PLL_ENABLE)) fprintf(stderr, "FDI RX PLL assertion failure, should be active but is disabled\n");
}
				 ...
				 enum plane plane)
{
	...
	reg = DSPCNTR(plane);
	val = I915_READ(reg);
	if (!(val & DISPLAY_PLANE_ENABLE)) fprintf(stderr,
...
static void assert_pch_refclk_enabled(struct drm_i915_private *dev_priv)
{
	...
	if (!enabled) fprintf(stderr, "PCH refclk assertion failure, should be active but is disabled\n");
}
				   ...
				   enum pipe pipe, int reg)
{
	...
	if (DP_PIPE_ENABLED(val, pipe)) fprintf(stderr,
					...
					unsigned long latency_ns)
{
	...
	fprintf(stderr, "FIFO entries required for mode: %ld\n", entries_required);

	wm_size = fifo_size - (entries_required + wm->guard_size);

	fprintf(stderr, "FIFO watermark level: %ld\n", wm_size);
							 ...
							 int mem)
{
	...
	fprintf(stderr, "Unknown FSB/MEM found, disable CxSR\n");
...
static int i9xx_get_fifo_size(struct drm_device *dev, int plane)
{
	...
	fprintf(stderr, "FIFO size - (0x%08x) %s: %d\n", dsparb,
		      plane ? "B" : "A", size);
...
{
	fprintf(stderr, "SR watermark: display plane %d, cursor %d\n",
		      display_wm, cursor_wm);

	if (display_wm > display->max_wm) {
		fprintf(stderr, "display watermark is too large(%d/%ld), disabling\n",
			      display_wm, display->max_wm);
		return false;
	}

	if (cursor_wm > cursor->max_wm) {
		fprintf(stderr, "cursor watermark is too large(%d/%ld), disabling\n",
			      cursor_wm, cursor->max_wm);
		return false;
	}

	if (!(display_wm || cursor_wm)) {
		fprintf(stderr, "SR latency is 0, disabling\n");
		return false;
	}
...
bool ironlake_set_drps(struct drm_device *dev, u8 val)
{
	...
	if (rgvswctl & MEMCTL_CMD_STS) {
		fprintf(stderr, "gpu busy, RCS change rejected\n");
